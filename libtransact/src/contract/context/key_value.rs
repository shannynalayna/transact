    ...
    K: Eq + Hash,
{
    ...
    /// of natural keys.
    ///
    /// Returns a HashMap links a normalized key with a HashMap. This associated HashMap represents
...
mod tests {
    ...
    /// Create simple state backed by a HashMap.
    struct TestState {
        state: HashMap<String, Vec<u8>>,
    }

    /// Implements the simple state with basic methods to get, set, and delete state values.
    impl TestState {
        pub fn new() -> Self {
            TestState {
                state: HashMap::new(),
    ...
    /// Implements the TransactionContext trait for the TestContext.
    impl TransactionContext for TestContext {
        fn get_state_entries(
            &self,
            addresses: &[String],
    ...
    /// Create a HashMap from `key` and `value` to create a StateEntryValue message for a
    /// StateEntry message.
    fn create_entry_value_map(key: String, value: ValueType) -> HashMap<String, ValueType> {
        let mut value_map = HashMap::new();
        value_map.insert(key, value);
