...
fn calculate_hash_lengths(
    ...
    if prefix_length > ADDRESS_LENGTH {
        return Err(AddresserError {
            message: format!("Prefix length is greater than {}", ADDRESS_LENGTH),
        });
    }
    match (first_length, second_length) {
        (Some(first), Some(second)) => {
            // Validate the custom hash lengths plus the prefix length is not greater than the
            // ADDRESS_LENGTH.
        ...
        (None, Some(second)) => {
            // Validate the custom hash length plus the prefix length is not greater than the
            // ADDRESS_LENGTH.
            if prefix_length + second > ADDRESS_LENGTH {
                return Err(AddresserError {
                    message: format!(
                        "Prefix length and custom hash length combined are greater than {}",
                        ADDRESS_LENGTH
                    ),
                });
            }
            // If the prefix length and custom length are not greater than ADDRESS_LENGTH, the
        ...
        (None, None) => {
            ...
            let calculated_second = calculated_first.clone();
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "Prefix length and custom hash length combined are greater than 70")]
    ...
    fn test_invalid_first_custom_length_construction() {
        ...
        // Unwrap to validate that this will panic with the expected error message.
        addresser.unwrap();
    }
    ...
    #[test]
    #[should_panic(expected = "Prefix length and hash lengths combined are greater than 70")]
    /// This test constructs a TripleKeyHashAddresser with a 6 character `prefix` and an optional
    ...
    /// lengths and should return an error. Also validates the expected error message.
    ...
    #[test]
    #[should_panic(expected = "Prefix length is greater than 70")]
